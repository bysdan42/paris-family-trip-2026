...
function renderLibrary(){
 element('document-library').innerHTML=`<div class="library-grid">${Object.entries(documents).map(([id,doc])=>`<article class="library-card"><span class="status">${doc.status}</span><h3>${doc.title}</h3><p>${doc.description}</p>${doc.reference?`<p class="reference">מספר הזמנה: <b dir="ltr">${doc.reference}</b></p>`:''}<div class="doc-actions">${id==='planning'?`<a class="action-link" href="${doc.path}" download>הורדת יומן התכנון</a>`:`<a class="action-link" href="${doc.path}" target="_blank" rel="noopener noreferrer">פתיחת המסמך ↗</a><a class="secondary-link" href="${doc.path}" download>הורדה למכשיר ↓</a>`}</div></article>`).join('')}</div><div class="library-grid baggage-library">${renderBaggage(days[0].baggage)}${renderBaggage(days[days.length-1].baggage)}</div><div class="library-notice"><strong>לתשומת לבכם · יתרת דיסני לפי המכתב</strong><p>במכתב מ־7.7.2026 רשומה יתרה של <b dir="ltr">$3,915.33</b> לתשלום עד <b dir="ltr">18.9.2026</b>. זהו מצב ההזמנה בעת הפקת המכתב; אין כאן מידע אם היתרה שולמה מאז. <a href="./documents/disney-confirmation.pdf#page=4" target="_blank" rel="noopener noreferrer">פתיחת עמוד התשלום ↗</a></p></div><section class="detail-card"><h3>מסמכים שעדיין לא צורפו</h3><p>אישור חניה ב־Edgar, השכרת רכב, כרטיסי האייפל, שייט וכרטיסי מוזיאונים. ההסעות מאורלי ואליו מאושרות ומופיעות בימים הרלוונטיים.</p></section>`;
}
