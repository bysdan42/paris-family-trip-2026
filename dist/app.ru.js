...
function renderBaggage(baggage){
 if(!baggage)return '';
 return `<section class="detail-card baggage-card"><h3>${escapeHtml(baggage.title)}</h3><strong class="baggage-total">${escapeHtml(baggage.total)}</strong><div class="detail-row"><small>Оплаченные нормы</small>${baggage.allowances.map(text=>`<p>${escapeHtml(text)}</p>`).join('')}</div>${baggage.sections.map(section=>`<div class="detail-row"><small>${escapeHtml(section.title)}</small><p>${escapeHtml(section.text)}</p></div>`).join('')}<p class="missing-note">${escapeHtml(baggage.sourceNote)}</p><div class="source-links">${baggage.sources.map(source=>externalLink(source.url,source.title,'')).join('')}</div></section>`;
}
...
function renderLibrary(){
 element('document-library').innerHTML=`<div class="library-grid">${Object.entries(documents).map(([id,doc])=>`<article class="library-card"><span class="status">${doc.status}</span><h3>${doc.title}</h3><p>${doc.description}</p>${doc.reference?`<p class="reference">Номер бронирования: <b dir="ltr">${doc.reference}</b></p>`:''}<div class="doc-actions">${id==='planning'?`<a class="action-link" href="${doc.path}" download>Скачать исходный план</a>`:`<a class="action-link" href="${doc.path}" target="_blank" rel="noopener noreferrer">Открыть документ ↗</a><a class="secondary-link" href="${doc.path}" download>Скачать на устройство ↓</a>`}</div></article>`).join('')}</div><div class="library-grid baggage-library">${renderBaggage(days[0].baggage)}${renderBaggage(days[days.length-1].baggage)}</div><div class="library-notice"><strong>Обратите внимание · остаток оплаты Disney по письму</strong><p>В письме от 7.7.2026 указан остаток <b dir="ltr">$3,915.33</b> к оплате до <b dir="ltr">18.9.2026</b>. Это состояние бронирования на дату письма; сведений о последующей оплате нет. <a href="./documents/disney-confirmation.pdf#page=4" target="_blank" rel="noopener noreferrer">Открыть страницу оплаты ↗</a></p></div><section class="detail-card"><h3>Документы, которых пока нет</h3><p>Подтверждение парковки Edgar, аренда машины, билеты на Эйфелеву башню, прогулку по Сене и в музеи. Трансферы из Орли и обратно подтверждены и указаны в соответствующих днях.</p></section>`;
}
